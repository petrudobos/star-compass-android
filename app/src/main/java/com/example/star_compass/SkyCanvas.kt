    ...
    Canvas(
        ...
        fun drawSkyText(text: String, x: Float, y: Float, paint: Paint, offsetBelow: Float = 0f) {
            drawIntoCanvas { canvas ->
                ...
                // Rotate text 90 degrees CCW for landscape reading
                canvas.nativeCanvas.rotate(-90f, x, y)
                // Draw text with horizontal offset (which is vertical relative to screen after rotation)
                canvas.nativeCanvas.drawText(text, x + offsetBelow, y, paint)
...
fun projectToScreen(
    ...
    val worldX = -(sin(azRad) * cos(altRad))  // East
    val worldY = -(cos(azRad) * cos(altRad))  // North
    ...
    if (screenZ_vec > 0) {
        ...
        val screenY = centerY + (screenY_vec / screenZ_vec) * scaleY * PROJECTION_SCALE
